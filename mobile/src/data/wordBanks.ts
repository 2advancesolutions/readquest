 ...
 * Mobile port: localStorage helpers removed (use src/lib/storage instead).
 */
...
export interface ContextItem {
  sentence: string       // target word wrapped in *asterisks*
  targetWord: string
  correct: string
  choices: string[]
  grade: number
...
export function calcXP(level: number, streakCount: number): number {
  const base = level <= 10 ? 5 : level <= 30 ? 8 : level <= 60 ? 12 : level <= 85 ? 18 : 25
